...
/// (skills deal fixed damage) without touching per-entity balance. Shared with
/// the gauntlet (`gauntlet.rs`) so both modes speak the same scale.
pub(crate) const HP_SCALE: f32 = 2.0;
...
pub(crate) const SPAWN_MP: f32 = 100.0;
pub(crate) const MP_REGEN: f32 = 2.0;

pub(crate) fn push_skill(skills: &mut Vec<Skill>, s: Skill) -> SkillId {
